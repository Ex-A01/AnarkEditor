...
namespace EvershadeEditor.LM2 {
    public class HashBin {
        ...
        string HashPath = Path.Combine(AppContext.BaseDirectory, "Hashes.bin");

        private void Load() {
            if (!Path.Exists(HashPath)) { throw new Exception("Hash file does not exist (Hashes.bin)"); }

            using (FileStream stream = new FileStream(HashPath, FileMode.Open, FileAccess.Read))
