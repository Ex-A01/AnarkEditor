...
namespace EvershadeEditor.LM2 {
    public class LM2File {
        ...
        List<string> Name_LocatedChunks = new();
        List<string> Value_LocatedChunks = new();
        ...
        public LM2File(string filePath) {
            ...
            /*
            foreach (ChunkEntry chunk in Chunks.Where(x => x.Type == (ushort)ChunkType.MaterialEffects).ToList()) {
                LocateChunk(chunk);
            }

            for (int i = 0; i < Name_LocatedChunks.Count; i++) {
                Console.WriteLine($"{Name_LocatedChunks[i]} = {Value_LocatedChunks[i]}");
            }
            */
        }

        private void LocateChunk(ChunkEntry chunk) {
            if (chunk.Type == (ushort)ChunkType.MaterialShaderName) {
                Name_LocatedChunks.Add(Encoding.Default.GetString(chunk.Data));
                return;
            }

            if (chunk.Type == (ushort)ChunkType.MaterialShaderHeader) {
                Value_LocatedChunks.Add(String.Join(" ", chunk.Data.Select(b => b.ToString("X2"))));
                return;
            }

            if (!chunk.HasChildren) { return; }

            foreach (ChunkEntry childChunk in chunk.Children) {
                LocateChunk(childChunk);
            }
        }
        ...
        public void Load() {
            ...
            foreach (ChunkEntry chunk in Chunks) {
                uint chunkMax = chunk.Offset + GetChunkBlock(chunk.BlockIndex).Offset + chunk.Size;
                uint blockMax = GetChunkBlock(chunk.BlockIndex).Offset + GetChunkBlock(chunk.BlockIndex).DecompressedSize;

                if (chunkMax > blockMax) {
                    Console.WriteLine($"Block at index {chunk.Index} is out of range: {chunkMax} > {blockMax}");
                }
            }

            /*
            byte[] ddsData = File.ReadAllBytes(Path.ChangeExtension(DataPath, ".dds"));
            ((TextureChunk)Textures[8]).SetTexture(ddsData);

            UNSAFESetChunkData(Textures[8].Children[1], ddsData, (int)Textures[8].Index);
            ((TextureChunk)Textures[8]).TexSize = (uint)ddsData.Length;
            ((TextureChunk)Textures[8]).TexWidth = (ushort)1024;
            ((TextureChunk)Textures[8]).TexHeight = (ushort)1024;
            ((TextureChunk)Textures[8]).SetMipmapLevel(8);
            ((TextureChunk)Textures[8]).Write();
            */
        }
        ...
        private void BASICSetChunkData(ChunkEntry chunk, byte[] data, bool aligned = false) {
        ...
        public void SetChunkData(ChunkEntry chunk, byte[] data, int parentIndex, bool aligned = false) {
            ...
            for (int i = parentIndex + 1; i < Chunks.Count; i++) {
                if (GetChunkBlockIndex(Chunks[i].BlockIndex) == blockIndex) {
                    OffsetChunk(Chunks[i], sizeOffset);
                }
            }
        ...
        private void OffsetChunk(ChunkEntry chunk, int offset) {
            if (chunk.HasChildren) {
                foreach (ChunkEntry childChunk in chunk.Children) {
                    OffsetChunk(childChunk, offset);
                }
            } else {
